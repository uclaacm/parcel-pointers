        ...
        <div className="selectdiv">
            ...
            <div className="arrow">
                <svg width="14" height="8" viewBox="0 0 14 8" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M13 1L7 7L0.999999 1" stroke="#868686" stroke-linecap="round"/>
                </svg>
            </div>
            <div className={"options " + (!showOptions ? "hidden": "")}>
                {props.options.map(({id, name, displayName}) => <span onClick={() => selectOption({id, name, displayName})} className={"option " + ((value.name == name) && "selected")}>{displayName}</span>)}
            </div>
        </div>
    );
}
